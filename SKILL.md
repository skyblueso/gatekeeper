...
6. **Are there patterns the scanner can't see?** The scanner uses regex + AST. It cannot follow data flow across files, track tainted variables through function calls, or evaluate runtime behavior. If you see a function that takes user input in file A and passes it to eval() in file B, the scanner won't connect those dots. You can. Flag cross-file risks the scanner missed.
