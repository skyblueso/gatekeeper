...
The scanner is ~4,500 lines across 5 modules in the `gatekeeper_scanner` package: `core.py` (scanner engine, CLI, verification, scoring), `ast_scanner.py` (AST-based Python analysis), `patterns.py` (all detection patterns), `models.py` (data models), and `reporter.py` (terminal output, SARIF generation). Tests are in `test_gatekeeper.py`.
...
**Categories:** `EXECUTION`, `INJECTION`, `FILESYSTEM`, `NETWORK`, `PERMISSION`, `SECRET`, `OBFUSCATION`, `DEPENDENCY`, `LICENSE`, `MCP`
